...
def test_provenance_structure():
    ...
    for rec in data["records"]:
        ...
        # New required fields
        assert "verification_status" in rec, f"Record {rec['id']} missing verification_status"
        assert rec["verification_status"] in VALID_STATUSES, f"Invalid status: {rec['verification_status']}"
        assert "claim_scope" in rec and len(rec["claim_scope"]) > 10, f"Record {rec['id']} missing claim_scope"
